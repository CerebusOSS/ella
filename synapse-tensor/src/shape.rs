...
impl Axis {
    #[inline]
    pub(crate) fn index<S>(&self, shape: &S) -> usize
    where
        S: Shape,
    {
    ...
    pub(crate) fn insert_index<S>(&self, shape: &S) -> usize
    where
        S: Shape,
    {
...
//           B: IndexValue,
// {
//     fn index_checked(&self, shape: &Const<2>) -> Option<usize> {
//         todo!()

//         // strides(&shape).zip([self.0, self.1]).map
//     }

//     fn index_unchecked(&self, shape: &Const<2>) -> usize {
//         self.1.abs_index(shape[1]) + self.0.abs_index(shape[0]) * shape[1]
//     }
// }

// #[inline]
// pub fn abs_index(len: usize, index: isize) -> usize {
//     if index < 0 {
//         len - (-index as usize)
//     } else {
//         index as usize
...
// fn strides<S: Shape>(shape: &S) -> impl Iterator<Item=usize> + '_ {
//     let mut strides = S::zeros(shape.ndim());
//     if shape.as_ref().iter().all(|&d| d != 0) {

//     }
//     shape.as_ref().iter().scan(1, |stride, &dim| {
//         *stride *= dim;
//         Some(*stride)
//     })
// }
