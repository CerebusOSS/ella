...
use sha2::Sha256;
use synapse_common::OffsetDateTime;
use synapse_engine::{engine::SynapseState, SynapseConfig};
use tonic::service::Interceptor;
use uuid::Uuid;

#[derive(Debug, Clone)]
pub(crate) struct ConnectionState {
    state: Arc<Mutex<SynapseState>>,
}

impl ConnectionState {
    pub fn new(state: SynapseState) -> Self {
        Self {
            state: Arc::new(Mutex::new(state)),
        }
    }

    pub fn read(&self) -> SynapseState {
        self.state.lock().unwrap().clone()
    }

    pub fn set_config(&self, config: SynapseConfig) {
...
impl ConnectionManager {
    pub fn new(auth: Arc<AuthProvider>, state: SynapseState) -> Self {
        Self {
            auth,
            state,
