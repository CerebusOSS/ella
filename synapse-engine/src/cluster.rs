...
#[derive(Debug)]
pub struct SynapseCluster {
    catalogs: DashMap<Id<'static>, Arc<SynapseCatalog>>,
...
impl SynapseCluster {
    ...
    pub fn catalogs(&self) -> Vec<Arc<SynapseCatalog>> {
        self.catalogs.iter().map(|c| c.value().clone()).collect()
    }

    pub fn catalog<'a>(&self, id: impl Into<Id<'a>>) -> Option<Arc<SynapseCatalog>> {
        let id: Id<'a> = id.into();
    ...
    pub async fn create_catalog<'a>(
        ...
        match (if_not_exists, self.catalog(id.as_ref())) {
            ...
            (true, None) | (false, None) => {
                let catalog = Arc::new(SynapseCatalog::new(
                    id.clone().into(),
                    self.log.clone(),
                    self.root.clone(),
                ));
    ...
    pub(crate) async fn register(
        &self,
        id: Id<'static>,
        catalog: Arc<SynapseCatalog>,
    ) -> crate::Result<Option<Arc<SynapseCatalog>>> {
        self.log
    ...
    pub(crate) fn load(&self, snapshot: &Snapshot, state: &SynapseState) -> crate::Result<()> {
        for catalog in &snapshot.catalogs {
            self.catalogs.insert(
                catalog.id.clone().into(),
                Arc::new(SynapseCatalog::load(catalog, state)?),
            );
        }
        self.resolve(state)?;
        Ok(())
...
impl CatalogList for SynapseCluster {
